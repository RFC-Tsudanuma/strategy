...
def declare_actions(*actions):
    ...
    if current_domain == None:
        raise Exception("cannot declare actions until a domain has been created.")
...
def declare_commands(*commands):
    ...
    """
    if current_domain == None:
        raise Exception("cannot declare commands until a domain has been created.")
...
def declare_task_methods(task_name, *methods):
    ...
    """
    if current_domain == None:
        raise Exception("cannot declare methods until a domain has been created.")
...
def run_lazy_lookahead(state, todo_list, max_tries=10):
    ...
    for tries in range(1, max_tries + 1):
        ...
        if plan == False or plan == None:
            if verbose >= 1:
                raise Exception("run_lazy_lookahead: find_plan has failed")
            return state
        if plan == []:
            if verbose >= 1:
                print(
                    "RLL> Empty plan => success", f"after {tries} calls to find_plan."
                )
            if verbose >= 2:
...
print("Messages from find_plan will be prefaced with 'FP>'.")
print("Messages from run_lazy_lookahead will be prefaced with 'RLL>'.")
